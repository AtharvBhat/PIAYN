...
class SCHead(nn.Module):
    def __init__(self, config, isPerceiver = False):
        super().__init__()
        self.pooling_mode = config["pooling_mode"]
        
        if isPerceiver:
            self.mlpblock = nn.Sequential(
                nn.Linear(config["num_tokens"], config["transformer_hidden_dim"]),
                nn.ReLU(),
...
class ModelForSC(nn.Module):
    def __init__(self, config, percModel = None, isPerceiver = False):
        super().__init__()
        ...
        if percModel == None:
            ...
            self.model = percModel
        
        self.seq_classifer = SCHead(config, isPerceiver)
...
class SCHeadDual(nn.Module):
    def __init__(self, config, isPerceiver = False):
        ...
        if isPerceiver:
            self.mlpblock = nn.Sequential(
                nn.Linear(config["num_tokens"]*4,  config["transformer_hidden_dim"]),
                nn.ReLU(),
                nn.Linear(config["transformer_hidden_dim"], config["num_classes"])
            )
        else:
...
class ModelForSCDual(nn.Module):
    def __init__(self, config, percModel = None, isPerceiver = False):
        ...
        self.seq_classifer = SCHeadDual(config, isPerceiver)
