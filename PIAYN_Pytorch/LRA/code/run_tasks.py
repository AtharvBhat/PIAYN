...
parser.add_argument("--netid", type = str, help = "netid", dest = "netid", required = True)
...
SCRATCH = '/scratch/' + str(netID)

checkpoint_dir = SCRATCH + '/PIAYN/LRA/' +  "logs/"
...
if attn_type.startswith("perceiver"):
    percModel = PerceiverLM(
            **lra_config.config[task]["extra_attn_config"][attn_type]
        
            #num_tokens = 512,            # number of tokens
            #dim = 32,                    # dimension of sequence to be encoded
            #depth = 6,                   # depth of net
            #max_seq_len = 4000,          # maximum sequence length
...
if task == "retrieval":
    model = ModelForSCDual(model_config, percModel, isPerceiver)
else:
    model = ModelForSC(model_config, percModel, isPerceiver)
...
ds_iter = {
    "train":enumerate(DataLoader(LRADataset(f"{SCRATCH}/PIAYN/LRA/datasets/{task}.train.pickle", True), batch_size = training_config["batch_size"], drop_last = True)),
    "dev":enumerate(DataLoader(LRADataset(f"{SCRATCH}/PIAYN/LRA/datasets/{task}.dev.pickle", True), batch_size = training_config["batch_size"], drop_last = True)),
    "test":enumerate(DataLoader(LRADataset(f"{SCRATCH}/PIAYN/LRA/datasets/{task}.test.pickle", False), batch_size = training_config["batch_size"], drop_last = True)),
}
...
def step(component, step_idx):
    ...
    if component == "train":
        ...
        for partial_inputs in partial_inputs_list:
            print('partial inputs', partial_inputs.keys())
            print('i0', partial_inputs['input_ids_0'].shape)
            print('i1', partial_inputs['input_ids_1'].shape)
            print('m0', partial_inputs['mask_0'].shape)
            print('m1', partial_inputs['mask_1'].shape)
            print('l', partial_inputs['label'].shape)
